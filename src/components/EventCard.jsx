...
export const EventCard = ({ time, title, desc, badge, badgeColor, align }) => {
    ...
    return (
      ...
      ${align === 'left' ? 'md:mr-auto text-left md:text-right' : 'md:ml-auto text-left'}`}
        >
            <div className={`md:block absolute w-5 h-5 bg-white border-4 border-[var(--color-secondary)] rounded-full top-8 shadow-[0_0_0_4px_var(--color-bg)] z-200
        ${align === 'left' ? '-right-[57px]' : '-left-[57px]'}`}
            />
            {badge && (
                <span
                    className="inline-block text-white px-3 py-1 rounded-xl text-xs font-bold uppercase mb-2"
